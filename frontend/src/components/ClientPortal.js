...
const ClientPortal = ({ user }) => {
  ...
  const handleDownloadSource = async (documentId, filename) => {
    try {
      const response = await axios.get(
        `${API}/client/download-document/${documentId}`,
        { responseType: 'blob' }
      );
      ...
      alert('Error al descargar el documento: ' + (error.response?.data?.detail || error.message));
    }
  };
