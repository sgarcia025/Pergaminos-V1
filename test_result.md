...
agent_communication:
    - agent: "main"
    - message: "Implementadas las correcciones críticas solicitadas: (1) Endpoint rename documents - Cambiado de Form(...) a modelo Pydantic DocumentRename para aceptar JSON, (2) Configuración umbrales QA - Mejorados campos pass_threshold y critical_threshold con explicaciones visuales de rangos. Archivos modificados: /app/backend/server.py (modelo DocumentRename, endpoint rename actualizado), /app/frontend/src/components/QAAgents.js (sección umbrales mejorada), /app/frontend/src/components/ProjectDetail.js (rename ahora usa JSON). Necesita testing prioritario de estas correcciones críticas."
    - agent: "testing"
    - message: "✅ CRITICAL FIXES TESTING COMPLETED SUCCESSFULLY: Both critical corrections are working perfectly (7/7 tests passed, 100% success rate). DOCUMENT RENAME FIX: (1) PUT /api/documents/{document_id}/rename now accepts JSON payload {'new_name': 'nuevo_nombre.pdf'} instead of FormData - no more 'Field required' errors, (2) DocumentRename Pydantic model working correctly, (3) Special characters, spaces, and accents handled perfectly (tested 'Documento con Espacios y Acentos.pdf', 'Document_with-special@characters#2024.pdf', 'Contrato Número 123 - Empresa López & Asociados.pdf'). QA THRESHOLDS CONFIGURATION FIX: (4) Create QA Agent with custom thresholds working - pass_threshold=70, critical_threshold=85 saved and applied correctly, (5) Edit QA Agent thresholds working - successfully updated 70/85 → 65/80, scope changes, quality checks modifications, (6) Extreme threshold values accepted - pass_threshold=90, critical_threshold=95 working, (7) Threshold behavior validated: 0-64% = qa_failed, 65-79% = needs_review, 80-100% = qa_passed → processing. All critical fixes are production-ready and resolve the reported errors completely."
    - agent: "main"
